...
exports.shop1 = (req,res)=>{
  ...
  //sql5向shop_good插入数据
  const sql6 = 'update shop inner join shop_good on shop_cart_id = shop.id set number = ?, price = ? where shop_cart_id = ?'
  //更新购物车信息
      ...
      if(result2.length == 0) {
        db.query(sql3,[null,result1[0].id,req.body.num,req.body.commodity_price * req.body.num],(err3,result3)=>{
          ...
          db.query(sql4,result1[0].id,(err4,result4)=>{
            ...
            if(result4.length >= 1) {
              console.log(5);
              db.query(sql5,[null,result4[result4.length-1].id,req.body.commodity_id],(err5,result5)=>{
                if(err5) {
                  return res.cc(err5)
                }
                res.send({
...
exports.shop3 = (req,res)=>{
  ...
  const sql1 = 'update shop inner join shop_good on shop_cart_id = shop.id set number = ?, price = ? where shop_cart_id = ?'
  db.query(sql1,[req.body.number,parseFloat((req.body.number * req.body.price).toFixed(1)),req.body.shop_cart_id],(err1,result1)=>{
    if(err1) {
      return res.cc(err1)
    }
    else {
